...
import { addShopItem, getShopItems, deleteShopItem } from "@/lib/actions";
    ...
    if (file) {
      ...
      reader.onload = async () => {
        ...
        try {
          ...
          toast({ variant: "destructive", title: "Erreur", description: "Impossible d'ajouter l'article. Vérifiez la console pour plus de détails." });
          console.error(error);
        } finally {
          setIsSubmitting(false);
        }
    ...
    <div className="space-y-6">
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          ...
          <DialogContent className="sm:max-w-[525px]">
            <DialogHeader>
              ...
              <DialogDescription>
                Uploadez une image et ajoutez les détails de votre nouvel article. Taille max: 8Mo.
              </DialogDescription>
            </DialogHeader>
            <form onSubmit={handleSubmit}>
              <div className="grid gap-4 py-4">
                ...
                <div className="grid w-full items-center gap-1.5">
                  ...
                  <Input id="name" name="name" type="text" placeholder="Ex: Charnière" required />
                </div>
                <div className="grid w-full items-center gap-1.5">
                  <Label htmlFor="price">Prix (XOF)</Label>
                  <Input id="price" name="price" type="number" placeholder="Ex: 5000" required />
                </div>
                <div className="grid w-full items-center gap-1.5">
                  <Label htmlFor="description">Description</Label>
                  <Textarea id="description" name="description" placeholder="Ex: Charnière en inox pour porte." required />
                </div>
              </div>
              <DialogFooter>
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {[...Array(8)].map((_, i) => (
             <Card key={i}><CardContent className="p-0"><Skeleton className="aspect-[4/3] w-full rounded-t-lg" /></CardContent><CardHeader className="p-4 space-y-2"><Skeleton className="h-5 w-3/4" /><Skeleton className="h-5 w-1/2" /><Skeleton className="h-8 w-full" /></CardHeader><CardFooter className="p-4 pt-0 flex justify-end"><Skeleton className="h-9 w-9 rounded-full" /></CardFooter></Card>
          ))}
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
